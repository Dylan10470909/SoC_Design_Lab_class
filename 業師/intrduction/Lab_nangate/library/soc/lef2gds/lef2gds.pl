...
#    my $lx_hex=substr(sprintf("%8x",sprintf("%s",$lx/$U_UNIT)),-8);
#    my $rx_hex=substr(sprintf("%8x",sprintf("%s",$rx/$U_UNIT)),-8);
#    my $by_hex=substr(sprintf("%8x",sprintf("%s",$by/$U_UNIT)),-8);
#    my $ty_hex=substr(sprintf("%8x",sprintf("%s",$ty/$U_UNIT)),-8);
##    print("lx= $lx,$lx_hex,rx=$rx,$rx_hex,by=$by,$by_hex,$bby,,ty=$ty,$ty_hex\n");
#    my $xy=$lx_hex.$by_hex.$rx_hex.$by_hex.$rx_hex.$ty_hex.$lx_hex.$ty_hex.$lx_hex.$by_hex;
#    print gds pack("H88","002c1003".$xy);
#    my $endel = "00041100";
#    print gds pack("H8",$endel);
}
sub create_polygon(){
...
sub new_lib(){
    ...
    #my $user_unit=&float(0.001,8);
    my $user_unit="3e4189374bc6a7ef";
    #my $database_unit=&float(0.000000001,8);
    my $database_unit="3944b82fa09b5a51"; # 0.000000001 meter
    #my $unit ="00140305 3e4189374bc6a7ef 3944b82fa09b5a51";  # 0.001 0.000000001
...
sub float(){
    ...
    for(my $i=0; $i<$mantilength ; $i++){
        my $multi_value_manti=16*$value_manti;
        my $integer=int($multi_value_manti);
        my $fraction=$multi_value_manti-$integer;
