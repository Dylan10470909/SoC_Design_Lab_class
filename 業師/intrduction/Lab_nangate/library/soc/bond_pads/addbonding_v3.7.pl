...
my $iolist="io.list"; #default name
for (my $i=0;$i<@ARGV;$i++){
    if($ARGV[$i] =~ /-io/){
        $i++;
        $iolist=$ARGV[$i];
...
sub readIOPadList(){
    ...
    if($version != 3.6){
        print("\nERROR, io.list VERSION inconsistence. Required version: 3.6\n\n");
        exit;
    }
...
sub createAddBPadFile(){
    ...
    foreach my $side (qw (south east north west)){
        ...
        foreach $BPad (@$BPadList){
            ...
            print cmdfile ("addIoInst -cell $BPad->{cellname} -inst BPad_$BPad->{instname} -loc $BPadx $BPady -ori $dir\n");
        }
    }
...
sub createRouteBlkFile() {
    ...
    $westString= $westString."createRouteBlk -box $rsizechipleft $rsizechipdown $rsizeWestIOPadLength $rsizechipup -layer @$BlkLayers -name WRouteblk\n";
    $southString= $southString."createRouteBlk -box $rsizechipleft $rsizechipdown $rsizechipright $rsizeSouthIOPadLength -layer @$BlkLayers -name SRouteblk\n";
    $northString= $northString."createRouteBlk -box $rsizechipleft $routeBlk_N $rsizechipright $rsizechipup -layer @$BlkLayers -name NRouteblk\n";
    $eastString= $eastString."createRouteBlk -box $routeBlk_E $rsizechipdown $rsizechipright $rsizechipup -layer @$BlkLayers -name ERouteblk\n";
